...
export default class Game extends Component {
  handleNewJoiner = async () => {
    ...
    const startGame = await this.props.viralBankcontract.methods
      .startGame(patientZeroAddress)
      .send({ from: this.props.accounts[0] });
  ...
  handleDeposit = async () => {
    const deposit = await this.props.viralBankcontract.methods
      .buyInToRound()
      .send({ from: this.props.accounts[0] });
  };
    ...
    return (
      <div class="banner">
        <Container>
          <Row>
            <Col>
              <Card title={"Monthly Saving Target"} amount={"$20"} />
            </Col>
            <Col>
              <Card title={"Game Duration"} amount={"1"} subtext={"year"} />
            </Col>
            <Col>
              <Card title={"Starts In"} amount={"10"} subtext={"days"} />
            </Col>
          </Row>
          <Row style={{ marginTop: "20px" }}>
            <Button
              ...
              onClick={
                this.props.newJoiner ? this.handleNewJoiner : this.handleDeposit
              }
            >
